    ...
    for i, row in enumerate(rows, 1):
        ...
        if dep_dt and i <= len(EXPECTED_DEPARTURES_LIST):
            ...
            if dep_dt != expected_naive:
                errors.append(
                    f"{fname} row {i}: Departure {dep_dt} != expected {expected_naive}"
                )
        ...
        if case_id not in GC_CASES:
            ...
            try:
                ...
                if hs > MAX_HS_M:
                    errors.append(
                        f"{fname} row {i}: max_hs_m={hs:.1f} exceeds limit {MAX_HS_M} m"
                    )
            except ValueError:
                pass
...
def validate_file_b(
    ...
    if departure_dt and first_time and first_time != departure_dt:
        errors.append(
            f"{fname}: First waypoint time {first_time} != departure {departure_dt}"
        )
    ...
    if land_checker is not None and case_id not in GC_CASES:
        ...
        if land_rows:
            ...
            errors.append(
                f"{fname}: {len(land_rows)} waypoint(s) on land at row(s) {rows_str}"
            )
...
def _write_detailed_results(
    ...
    if plots:
        ...
        for title, img_b64 in plots:
            ...
            html_parts.append(
                f'<img src="data:image/png;base64,{img_b64}" alt="{safe_title}">'
            )
...
def score_submission() -> dict:
    ...
    for case in CASE_NAMES:
        ...
        if case_land_crossings > 0:
            all_errors.append(
                f"{case}: STRONG — {case_land_crossings} calculation points cross land"
            )
