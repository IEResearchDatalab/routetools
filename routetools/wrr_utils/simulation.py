...
def add_mid_points(route_waypoints: np.ndarray, num_mid_points: int) -> np.ndarray:
    """
    Add mid points to each segment of the route.
    ...
    route_waypoints : np.ndarray
        Array of waypoints
    num_mid_points : int
        Number of points to add to each segment
    """
    lats = route_waypoints[:, 0]
    lons = route_waypoints[:, 1]

    new_lats = np.array([lats[0]])
    new_lons = np.array([lons[0]])

    for i in range(len(lats) - 1):
        lats_mid_points = np.linspace(lats[i], lats[i + 1], num_mid_points + 2)[1:]
        lons_mid_points = np.linspace(lons[i], lons[i + 1], num_mid_points + 2)[1:]

        new_lats = np.concatenate((new_lats, lats_mid_points))
        new_lons = np.concatenate((new_lons, lons_mid_points))

    return np.column_stack((new_lats, new_lons))
...
def compute_speeds_given_coordinates_and_times(
    latitudes: np.array,
    longitudes: np.array,
    timestamps: np.array,
    ocean_data: Ocean,
    ...
    max_iter: int = 10,
    absolute_time_tol: float = 1,
    relative_time_tol: float = 1e-4,
    damping: float = 1.0,
    vel_min: float = 2,
    vel_max: float | None = None,
    land_penalization: float = 0,
) -> np.ndarray:
    """Approximate the speed (through water) of the ship.

    We use the Newton-Raphson method to approximate the speed given
    ...
    longitudes : np.array
        ...
        It shape can be (N_routes, N_points) or (N_points,)
    timestamps : np.array
       Timestamps of the route.
    ocean_data : Ocean
        Ocean object.
    step_size : float, optional
        Step size for the Newton-Raphson method, by default 1e-6
    max_iter : int, optional
        Maximum number of iterations, by default 10
    absolute_time_tol : float (seconds), optional
        Stop as soon as the total route time is this far from the target
        total time, by default 1.0
    relative_time_tol : float, optional
        Like `absolute_time_tol`, but in relative terms. By default 1e-4
    damping : float, optional
        Damping factor for the Newton-Raphson method, by default 1.0
    vel_min : float, optional
        Minimum speed in meters per second, by default 2
    vel_max : Optional[float], optional
        Maximum speed in meters per second, by default None

    Returns
    -------
    np.array
        Array containing the speed in meters per second for the given
        lat/lon coordinates. It will have the same shape as the input
        latitudes.
    """
    input_num_dims = len(latitudes.shape)
    assert (
        input_num_dims == 1 or input_num_dims == 2
    ), "Input arrays must have 1 or 2 dimensions"

    latitudes = np.atleast_2d(latitudes)
    longitudes = np.atleast_2d(longitudes)
    timestamps = np.atleast_2d(timestamps)
    lat0, lat1 = latitudes[..., :-1], latitudes[..., 1:]
    lon0, lon1 = longitudes[..., :-1], longitudes[..., 1:]

    # Target times
    t_target = (
        np.diff(timestamps, axis=-1).astype("timedelta64[us]").astype(float) / 1e6
    )

    # Function mapping speed to time
    def f(v: np.ndarray) -> np.ndarray:
        t = compute_times_linalg(
            lat0,
            lon0,
            lat1,
            lon1,
    ...
    # Initial guesses, use speed over ground
    d = distance_from_points(
        lat0,
        lon0,
        lat1,
    ...
    total_time = np.sum(t_target)  # The route should take this much in total

    # Iterative Newton-Raphson method
    for _it in range(max_iter):
        fv = f(vel)  # Traversal time for each segment
        fvprime = (f(vel + step_size) - fv) / step_size
        vel = vel - damping * fv / fvprime
        vel = np.clip(vel, a_min=vel_min, a_max=vel_max)

        # Early stopping criteria
        abs_error = np.abs(np.sum(fv + t_target) - total_time)
        if abs_error < absolute_time_tol:
            break
        if abs_error / total_time < relative_time_tol:
            break
