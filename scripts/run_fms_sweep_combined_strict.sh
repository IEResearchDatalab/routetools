#!/bin/bash
# ── Strict FMS refinement for sweep_combined output ──
#
# Applies FMS to output/sweep_combined and writes output/sweep_combined_fms_strict.
...
#   nohup bash scripts/run_fms_sweep_combined_strict.sh > output/sweep_combined_fms_strict.log 2>&1 &
#
# Monitor:
#   tail -f output/sweep_combined_fms_strict.log
#   cat output/fms_sweep_combined_strict.pid
...
LOGFILE="output/sweep_combined_fms_strict.log"
PIDFILE="output/fms_sweep_combined_strict.pid"
INPUT_DIR="output/sweep_combined"
OUTPUT_DIR="output/sweep_combined_fms_strict"
...
echo "============================================"
echo "Strict FMS sweep_combined runner started"
echo "Date:         $(date)"
echo "Host:         $(hostname)"
echo "PID:          $BASHPID"
...
echo "Wind limit:   $TWS_LIMIT"
echo "Wave limit:   $HS_LIMIT"
...
while (( attempt < MAX_RETRIES )); do
    ...
    if (( exit_code == 0 )); then
        echo ""
        echo "============================================"
        echo "Strict FMS completed successfully: $(date)"
        echo "Output: $OUTPUT_DIR"
        echo "============================================"
        rm -f "$PIDFILE"
        exit 0
    ...
    echo ""
    echo "WARNING: Strict FMS exited with code ${exit_code} at $(date)"
...
echo ""
echo "ERROR: Strict FMS failed after ${MAX_RETRIES} attempts. Giving up."
rm -f "$PIDFILE"
exit 1