...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "codabench" / "scoring_program"))
...
CODABENCH_REF = Path(__file__).resolve().parent.parent / "codabench" / "reference_data"
TRACKS_DIR = Path(__file__).resolve().parent.parent / "output" / "swopp3_0125_rust" / "tracks"
DTFMT = "%Y-%m-%d %H:%M:%S"
...
def main():
    ...
    for d in (CODABENCH_REF, DATA_DIR):
        ...
        if wf.exists() and vf.exists():
            wind_files = [str(wf)]
            wave_files = [str(vf)]
            for suf in (f"era5_wind_{corridor}_2025_01.nc",
                        f"era5_wind_{corridor}_2025.nc"):
                p = d / suf
                if p.exists():
                    wind_files.append(str(p))
                    break
            for suf in (f"era5_waves_{corridor}_2025_01.nc",
    ...
    curve_jax = jnp.array([[lon, lat] for lat, lon in zip(lats, lons)],
                           dtype=jnp.float32)[None, :, :]  # (1, L, 2)
    ...
    energy_jax = float(cost_function_rise(
        windfield=windfield,
        curve=curve_jax,
    ...
    dx_m, dy_m = haversine_meters_components(
        curve_jax[0, :-1, 1], curve_jax[0, :-1, 0],
        curve_jax[0, 1:, 1], curve_jax[0, 1:, 0],
    )
    ...
    segments_to_show = list(range(min(5, n_seg))) + list(range(max(0, n_seg - 5), n_seg))
    ...
    for i in segments_to_show:
        print(f"{i:4d} | {float(u10_jax[i]):10.4f} {float(u10_np[i]):10.4f} "
    ...
    print("-" * 80)
    for i in segments_to_show:
        print(f"{i:4d} | {float(bearing_jax[i]):10.4f} {float(bearing_np[i]):10.4f} "
    ...
    print("-" * 80)
    for i in segments_to_show:
        print(f"{i:4d} | {float(tws_jax[i]):10.4f} {float(tws_np[i]):10.4f} "
    ...
    print("-" * 80)
    for i in segments_to_show:
        print(f"{i:4d} | {float(hs_jax[i]):10.4f} {float(swh_np[i]):10.4f} "
    ...
    print("-" * 80)
    for i in segments_to_show:
        print(f"{i:4d} | {float(v_mps_jax[i]):10.4f} {float(v_mps_np[i]):10.4f} "
    ...
    print(f"  bearing: {np.sqrt(np.mean((np.array(bearing_jax) - bearing_np)**2)):.6f}")
