#!/bin/bash
#SBATCH --job-name=swopp3_nopen
#SBATCH --partition=cpu
#SBATCH --nodes=1
#SBATCH --cpus-per-task=64
#SBATCH --mem=128G
#SBATCH --time=2-00:00:00
#SBATCH --output=slurm_nopen_%j.out
#SBATCH --error=slurm_nopen_%j.err

# ── SWOPP3 full run: no weather penalization (Δt₁ = 2 h) ──
#
# Baseline run with all weather penalty weights at 0.  The optimizer is
# free to route through severe weather if it reduces energy consumption.
# n-points per corridor: Atlantic=178 (354h/2h+1), Pacific=293 (583h/2h+1).
#
# Submit:  sbatch scripts/swopp3_slurm_no_penalty.sh
...
OUTDIR="output/swopp3_no_penalty"
DT_EVAL=30   # Δt₂ = 30 min evaluation grid
DIST_PW=10.0  # distance-to-land penalty (always active)
mkdir -p "$OUTDIR"

echo "======================================"
echo "SWOPP3 no-penalty run on $(hostname)"
echo "Date:     $(date)"
echo "CPUs:     ${SLURM_CPUS_PER_TASK}"
echo "JAX:      CPU mode"
echo "Data:     ${DATA}"
echo "Output:   ${OUTDIR}"
echo "Weather:  ALL ZERO"
...
echo ""
echo "======================================"
echo "SWOPP3 no-penalty run completed at $(date)"
echo "======================================"
echo ""
echo "Output files:"
