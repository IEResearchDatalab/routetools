#!/usr/bin/env python3
r"""Atlantic parameter sweep — validate Pacific-tuned parameters.

Runs a grid search over weather-penalty and exploration parameters on
Atlantic noWPS departures to check if Pacific-derived recommendations
generalise across ocean basins.

Uses all 6 detour departures (delta > 10%) plus a sample of 14
representative departures spanning the full delta range, for 20 total.
...
Grid:
    ...
    = 5 × 3 × 2 × 2 = 60 configs × 20 departures = 1200 runs
...
SIGMA0_VALUES = [0.1, 0.3]
K_VALUES = [10, 15]

CASE_ID = "AO_noWPS"
GC_CASE_ID = "AGC_noWPS"
...
def main() -> None:
    ...
    target_deps = [(idx, all_departures[idx]) for idx in ALL_DEPARTURES]
